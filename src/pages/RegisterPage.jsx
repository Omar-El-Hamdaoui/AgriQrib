    ...
    {Array.from({ length: total }).map((_, idx) => (
      <div key={idx} className="flex items-center gap-2">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300
          ${idx < visualStep ? 'bg-[#2D5016] text-white'
            : idx === visualStep ? 'bg-[#2D5016] text-white ring-4 ring-[#2D5016]/20'
              : 'bg-stone-200 text-stone-500'}`}
        >
          {idx < visualStep ? <Icons.Check /> : idx + 1}
        </div>
        {idx < total - 1 && (
          <div className={`h-0.5 w-8 transition-all duration-300 ${idx < visualStep ? 'bg-[#2D5016]' : 'bg-stone-200'}`} />
        )}
      </div>
    ))}
  </div>
...
const StepIdentity = ({ data, errors, setField }) => (
  <div>
    ...
    <p className="text-stone-500 mb-6">Renseignez vos coordonnées personnelles.</p>
    <div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <Input label="Prénom *" placeholder="Marie" value={data.firstName}
            onChange={e => setField('firstName', e.target.value)} maxLength={100} />
          <FieldError msg={errors.firstName} />
        </div>
        <div>
          <Input label="Nom *" placeholder="Dupont" value={data.lastName}
            onChange={e => setField('lastName', e.target.value)} maxLength={100} />
          <FieldError msg={errors.lastName} />
        </div>
      </div>
      <div>
        <Input label="Adresse email *" type="email" placeholder="marie@exemple.fr"
          icon={<Icons.User />} value={data.email}
          onChange={e => setField('email', e.target.value)} />
        <FieldError msg={errors.email} />
      </div>
      <div>
        <Input label="Téléphone (optionnel)" type="tel" placeholder="+33 6 12 34 56 78"
          value={data.phone} onChange={e => setField('phone', e.target.value)} maxLength={20} />
...
const StepPassword = ({ data, errors, setField }) => {
  ...
  return (
    <div>
      ...
      <p className="text-stone-500 mb-6">Choisissez un mot de passe sécurisé.</p>
      <div className="space-y-4">
        <div>
...
const StepFarm = ({ data, errors, setField, toggleCertification }) => (
  <div>
    ...
    <p className="text-stone-500 mb-6">Informations sur votre ferme.</p>
    <div className="space-y-4">
      <div>
        <Input label="Nom de la ferme *" placeholder="Ferme du Soleil Levant"
          value={data.farmName} onChange={e => setField('farmName', e.target.value)} maxLength={200} />
        <FieldError msg={errors.farmName} />
      </div>
      <div>
        <label className="block text-sm font-medium text-stone-700 mb-1.5">Description (optionnelle)</label>
        <textarea value={data.description} onChange={e => setField('description', e.target.value)}
          placeholder="Parlez de votre exploitation…" rows={3}
      ...
      <div>
        <Input label="Adresse *" placeholder="12 Route des Champs"
          value={data.address} onChange={e => setField('address', e.target.value)} />
        <FieldError msg={errors.address} />
      </div>
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <Input label="Ville *" placeholder="Aix-en-Provence"
            value={data.city} onChange={e => setField('city', e.target.value)} maxLength={100} />
          <FieldError msg={errors.city} />
        </div>
        <div>
          <Input label="Code postal *" placeholder="13100"
            value={data.postalCode} onChange={e => setField('postalCode', e.target.value)} maxLength={5} />
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {CERTIFICATIONS.map(cert => (
            <button key={cert} type="button" onClick={() => toggleCertification(cert)}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="range" min={5} max={200} step={5} value={data.deliveryRadius}
            onChange={e => setField('deliveryRadius', e.target.value)} className="w-full accent-[#2D5016]" />
          <div className="flex justify-between text-xs text-stone-400 mt-1"><span>5 km</span><span>200 km</span></div>
          <FieldError msg={errors.deliveryRadius} />
        </div>
        <div>
          <Input label="Commande minimum (DH)" type="number" min={0} placeholder="0"
            value={data.minimumOrder} onChange={e => setField('minimumOrder', e.target.value)} />
      ...
      <div className="space-y-3">
        <label className="flex items-start gap-3 cursor-pointer group">
          ...
          <span className="text-sm text-stone-700">
            J'accepte les{' '}
            <a href="https://www.index-education.com/contenu/telechargement/doc/Modele_CGU_CLIENT.pdf"
              className="text-[#2D5016] underline hover:text-[#1e3a0f]">Conditions Générales d'Utilisation</a>{' '}
            et la{' '}
            <a href="https://www.economie.gouv.fr/politique-confidentialite"
              className="text-[#2D5016] underline hover:text-[#1e3a0f]">Politique de confidentialité</a>. *
          </span>
        </label>
...
const SuccessScreen = ({ data, setCurrentView }) => (
  <div className="text-center py-8">
    ...
    <p className="text-stone-500 mb-2">Votre compte a été créé avec succès.</p>
    <p className="text-sm text-stone-400 mb-8">
      Un email de vérification a été envoyé à <strong>{data.email}</strong>.
    </p>
    <div className="flex flex-col sm:flex-row gap-3 justify-center">
      <Button onClick={() => setCurrentView('home')}>Accéder à la marketplace</Button>
      {data.role === 'producer' && (
            ...
            <>
              ...
              <div className="mb-6">
                <div className="flex justify-between text-xs text-stone-400 mb-2">
                  <span>Étape {visualStep + 1} / {totalSteps}</span>
                  <span>{progressPercent}%</span>
                </div>
                <div className="h-1.5 bg-stone-200 rounded-full overflow-hidden">
                  <div className="h-full bg-[#2D5016] rounded-full transition-all duration-500"
                    style={{ width: `${progressPercent}%` }} />
                </div>
              </div>

              {/* Step indicator — dynamique selon le rôle */}
              <StepIndicator visualStep={visualStep} total={totalSteps} />
